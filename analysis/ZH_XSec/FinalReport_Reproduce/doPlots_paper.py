...
def runPlots(var,param,hsignal,hbackgrounds,extralab):
    ...
    leg.AddEntry(hbackgrounds["ZZ"][0],param.legend["ZZ"],"f")
    leg.AddEntry(hbackgrounds["WW"][0],param.legend["WW"],"f")
    leg.AddEntry(hbackgrounds["Other"][0],param.legend["Other"],"f")
    histos=[]
    colors=[]
    ...
    rt = "#sqrt{{s}} = {:.0f} TeV,   L = {:.1f} ab^{{-1}}".format(param.energy,intLumiab)

    if 'ee' in param.collider:
        lt = "FCC-ee Simulation (Delphes)"
        rt = "#sqrt{{s}} = {:.0f} GeV,   L = {:.1f} ab^{{-1}}".format(param.energy,intLumiab)
...
def drawStack(name, ylabel, legend, leftText, rightText, formats, directory, logY, stacksig, histos, colors, ana_tex, extralab):
    ...
    canvas = ROOT.TCanvas(name, name, 1200, 800)
    canvas.SetLogy(logY)
    canvas.SetTicks(1,1)
