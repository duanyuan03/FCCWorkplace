"""
Referee baseline scenario (ecm 365): NO BDT anywhere, single baseline region,
...
def build_lep(flavor):
    ...
    h_obs = hists[0].Clone("data_obs")
    hb = sum1d(base, LEP_BKG[flavor], sel, "recoil_m", "background", strict=False)
    hists.append(hb)
    h_obs.Add(hb)
    hists.append(h_obs)
    ...
    hists.append(sum1d(base, [f"wzp6_ee_{flavor}H_BES-higher-10pc_ecm{_ecm}"], sel, "recoil_m", "signal_BESUp"))
    hists.append(sum1d(base, [f"wzp6_ee_{flavor}H_BES-lower-10pc_ecm{_ecm}"], sel, "recoil_m", "signal_BESDown"))
    fOut = ROOT.TFile(f"{runDir}/datacard.root", "RECREATE")
    for h in hists: h.Write()
    fOut.Close()
